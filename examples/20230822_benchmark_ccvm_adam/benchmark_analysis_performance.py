...
def sub_scatter(baseline, data, subkey):
    ...
    plt.scatter(
        data["alpha"],
        data["r06"]["solution_performance"][subkey] / base,
        color=f"C{c}",
        marker=">",
    )
    plt.scatter(
        data["alpha"],
        data["r07"]["solution_performance"][subkey] / base,
        color=f"C{c}",
        marker="1",
    )
    plt.scatter(
        data["alpha"],
        data["r08"]["solution_performance"][subkey] / base,
        color=f"C{c}",
        marker="s",
    )
    plt.scatter(
        data["alpha"],
        data["r09"]["solution_performance"][subkey] / base,
        color=f"C{c}",
        marker="p",
    )
    plt.scatter(
        data["alpha"],
        data["r10"]["solution_performance"][subkey] / base,
        color=f"C{c}",
        marker=".",
    )
    return base
...
for beta2 in [0.999, 0.8]:  #
    for beta1 in [0.9, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9]:
        ...
        # plt.show()
        plt.savefig(
            f"{PLOT_DIR}performance_1beta{beta1:.03f}_2beta{beta2:.04f}_iter{iterations:06d}.png",
            dpi=500,
        )
        plt.close()
