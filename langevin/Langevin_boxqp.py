...
def calculate_grads(c, q_matrix, c_vector, rate):
...
def solve(n, q_mat, c_vector, q_scale, c_scale, pump_rate_flag, lr=0.09, n_iter=50000):
    """
    ...
    for i in range(n_iter):
        ...
        grads = calculate_grads(c, q_mat, c_vector, pump_rate)
        Wt = get_w(i, n_iter)
        c = c + lr * (grads + Wt)
...
def scale_coefs(q, c, scaling_val=None):
    if scaling_val:
        ...
        # c_scale = torch.sqrt(torch.sum(torch.abs(c))) * 0.5
        # print([q_scale, c_scale])
        c_scale = q_scale
        # q_scale = c_scale
    return q / q_scale, c / c_scale, q_scale, c_scale
    # return q / scaling_val, c / scaling_val
...
if __name__ == "__main__":
    ...
    # Specification of the instances to be solved
    ...
    iter_options = [15000]
    scaling_options = [2, 4, 10, None]
    lr_options = [0.0025, 0.005, 0.01]
    noises = [[0.02, 0.5], [0.002, 0.05], [0.0002, 0.005]]
    log_file_path_basic = (
        f"./results/results_Langevin_{post_method}_{device}_{instance_type}.txt"
    )
    ...
    if parameters_search == "key":
        ...
        j_options = [None]
        parameters_key = {
            "20": {"scale": 10, "noise": [0.02, 0.5], "lr": 0.01, "iter": 15000},
            "30": {"scale": None, "noise": [0.02, 0.5], "lr": 0.01, "iter": 15000},
            "40": {"scale": None, "noise": [0.02, 0.5], "lr": 0.01, "iter": 15000},
            "50": {"scale": None, "noise": [0.02, 0.5], "lr": 0.005, "iter": 15000},
            "60": {"scale": None, "noise": [0.02, 0.5], "lr": 0.01, "iter": 15000},
            "70": {"scale": None, "noise": [0.02, 0.5], "lr": 0.01, "iter": 15000},
        }
                ...
                for my_iter in iter_options:
                    ...
                    for my_lr in lr_options:
                        ...
                        for pump_rate_flag in [True]:
                            ...
                            log_file_basic.write("\t")
                            log_file_basic.write("{:.3f}".format(sol_time / batch_size))
                            log_file_basic.write("\n")
